...
var createAndSavePerson = (done) => {
  const cristina = new Person({ name: "Cristina", age: 33, favoriteFoods: ["pizza", "pasta", "focaccia", "cioccolato"]});

  cristina.save((err, data) => err ? done(err) : done(null, data));
};
...
var findPeopleByName = (personName, done) => {
  Person.find({ name: personName }, (err, people) => err ? done(err) : done(null, people));
};
...
var findPersonById = function(personId, done) {
  Person.findById(personId, (err, person) => err ? done(err) : done(null, person));
};
